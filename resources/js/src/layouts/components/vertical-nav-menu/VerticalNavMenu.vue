      ...
      <div @mouseenter="mouseEnter" @mouseleave="mouseLeave">
        ...
        <VuePerfectScrollbar ref="verticalNavMenuPs" class="scroll-area-v-nav-menu pt-2" :settings="settings" @ps-scroll-y="psSectionScroll" :key="$vs.rtl">
          <template v-for="(item, index) in menuItemsUpdated">
            ...
            <!-- Group Header -->
            <span v-if="item.header && !verticalNavMenuItemsMin && isRole(item.rol)" class="navigation-header truncate" :key="`header-${index}`">
            ...
            <template v-else-if="!item.header">
              ...
              <!-- Nav-Item -->
              <v-nav-menu-item
                v-if="!item.submenu && isRole(item.rol)"
                :key="`item-${index}`"
                :index="index"
              ...
              <!-- Nav-Group -->
              <template v-else-if="isRole(item.rol)">
                <v-nav-menu-group
                  :key="`group-${index}`"
